...
class DictBinaryTree:
    ...
    def height(self, node):
        """Permette di ritornare altezza associato ad un nodo"""
        if node == None:
            return None
        return node.info[2]
    ...
    def minKeySon(self, root):
        """Permette di ottenere il nodo con chiave piu' piccolo,
        partendo dal nodo root. Il nodo con chiave
        piu' piccolo e' quello che si trova piu' a destra possibile"""
        curr = root
    ...
    def searchNode(self, key):
        ...
        Ritorna il nodo, oppure None se non c'e' nodo associato a key"""
    ...
    def searchNodeAVL(self, key, value, height):
        """Permette di ricercare il nodo con chiave key all'interno del dizionario.
        Ritorna il nodo, oppure None se non c'e' nodo associato a key,value e height"""
        if self.tree.root == None:
            return None
        ...
        while curr != None:
            ...
            cv = self.value(curr)
            ch = self.height(curr)
            if key == ck and value == cv and height == ch:
                return curr
            elif key < ck:
                curr = curr.leftSon
            elif key > ck:
                curr = curr.rightSon
            elif key == ck and value == self.value(curr.leftSon) and height == self.height(curr.leftSon):
                curr = curr.leftSon
            elif key == ck and value == self.value(curr.rightSon) and height == self.height(curr.rightSon):
                curr = curr.rightSon
            else:
                return None
